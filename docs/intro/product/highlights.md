...
[查看完整支持列表](/docs/intro/appendix/db-support/index.md)

### 备端支持完整

满足任何兼容 `OCI` 或 `ODBC` 接口的关系型数据库，包括但不限于 Oracle、MySQL、PostgreSQL、SQL Server、DB2、OceanBase、TiDB、GaussDB、openGauss、武汉达梅 DM、人大金仓 Kingbase、LightDB、SUNDB、云和恩墨 MogDB 等。
...
通过 [FZS Web](/docs/fzs-web/index.md)，用户可以通过一键部署功能，快速部署 [FZS Web Server](/docs/fzs-web-server/index.md)，实现源备端的数据同步。

![FZS Web 一键部署功能演示截图](/img/intro/quick-deploy-demo.png)

### 配置链路

通过 [FZS Web](/docs/fzs-web/index.md)，用户可以通过配置链路功能，快速配置源备端的数据同步链路。

![FZS Web 数据链路配置界面演示截图](/img/intro/datalink-config-demo.png)

### 链路监控

通过 [FZS Web](/docs/fzs-web/index.md) 的仪表盘大屏，用户可以实时监控多条数据同步链路的同步计数、增量延时、链路状态等数据。

![FZS Web 仪表盘链路监控大屏截图](/img/intro/monitor-demo.png)

### 告警平台

通过 [FZS Web](/docs/fzs-web/index.md) 的告警平台，用户可以实时监控多条数据同步链路的告警信息，并通过邮件、短信、Webhook 等方式通知管理员用户。
