...
上传成功后，将授权文件绑定到对应的 Web Server 上，点击 **绑定** 完成操作。

![绑定 License 授权至 Web Server 的操作界面](/img/bind-license.jpg)

## 4. 添加数据节点

在页面左上角选中目标 Web Server（确认 IP 和端口与您部署的 Web Server 一致），然后填写数据节点名称，选择数据库类型，填入数据库连接串及 FZS 数据库连接用户，点击 **提交**。

![添加数据节点并填写数据库连接信息的配置界面](/img/quick/add-node.png)

## 5. 添加数据链路

点击 **添加链路**，填写链路名称，分别选择源端数据节点和目标端数据节点，点击 **提交**。
...
- **查看同步日志**
    - 选中对应链路，分别点击源端和目标端查看日志
  
      ![选中链路后查看源端和目标端同步日志的界面](/img/quick/log.png)
- **全同步完成标志**
    - 备端出现装载结束时间，说明全同步完成
  
      ![全量同步完成时备端出现装载结束时间的标志界面](/img/quick/full-sync.png)
- **查看增量同步的状态**
    - 选中对应链路，分别点击源端和目标端查看增量同步状态
...
- **数据比对**
    ...
    - 如果显示数据不一致，可点击卡片右上角修复，对数据不一致的表进行修复
      :::warning
      修复操作需要同时满足两个条件：目标端无同步延迟，且当前为非业务高峰期。
      :::
