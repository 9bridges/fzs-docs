...
当同步链路的源端为 Oracle 时，FZS 需要直接读取数据库的 redo 文件。因此，FZS Web Server 必须部署在数据库所在的**本地环境**（而非容器中）。支持 Linux、AIX 等操作系统（Windows 支持研发中）。

## 部署条件

- **硬盘空间**：单条链路至少预留同步数据量 1/3 的磁盘空间
- **内存与 CPU**：无硬性要求，机器性能越高同步效率越好
- **网络端口**：
  - 对外开放 8081 端口（HTTP 访问）
  - 对内开放所有端口（内部通信）
